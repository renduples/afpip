...
# AFPI GCP Deployment Script - Cloud Build Version
...
echo -e "${BLUE}   (Using Cloud Build - No Docker Required)${NC}"
echo -e "${BLUE}================================================${NC}"
echo ""
...
echo -e "\n${BLUE}Building and deploying backend with Cloud Build...${NC}"
...
# Note: Using sqlite for initial deployment - will migrate to Cloud SQL later
gcloud run deploy $BACKEND_SERVICE \
    ...
    --max-instances=10 \
    --timeout=300 \
    --set-env-vars=ENVIRONMENT=production,DATABASE_URL=sqlite:///./afpi.db,REDIS_URL=redis://localhost:6379,SECRET_KEY=temp-secret-key,JWT_SECRET_KEY=temp-jwt-secret,GCP_PROJECT_ID=${PROJECT_ID},VERTEX_AI_PROJECT=${PROJECT_ID} \
    --quiet
...
echo -e "\n${BLUE}Building and deploying frontend with Cloud Build...${NC}"
echo -e "${YELLOW}This will take 5-8 minutes...${NC}"
cd frontend
gcloud builds submit \
